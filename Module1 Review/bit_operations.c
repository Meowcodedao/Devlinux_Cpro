...
// 1️⃣ Bật (set) một bit cụ thể
uint32_t set_bit(uint32_t n, uint8_t pos) {
    return n | (1U << pos);
}

// 2️⃣ Tắt (clear) một bit cụ thể
uint32_t clear_bit(uint32_t n, uint8_t pos) {
    return n & ~(1U << pos);
}

// 3️⃣ Đảo (toggle) một bit cụ thể
uint32_t toggle_bit(uint32_t n, uint8_t pos) {
    return n ^ (1U << pos);
}

// 4️⃣ Kiểm tra một bit cụ thể đã được bật hay chưa
bool is_bit_set(uint32_t n, uint8_t pos) {
    return (n >> pos) & 1U;
}

// 5️⃣ Hoán đổi hai số mà không cần biến thứ ba
...
// 6️⃣ Kiểm tra một số có phải là lũy thừa của 2 hay không
bool is_power_of_two(uint32_t n) {
    return n && !(n & (n - 1));
}

// 7️⃣ Đếm số bit 1 trong biểu diễn nhị phân
...
// 8️⃣ Kiểm tra số chẵn hay lẻ
bool is_even(uint32_t n) {
    return !(n & 1);
}

// 9️⃣ Tìm bù 2 (two’s complement)
int32_t twos_complement(int32_t n) {
    return (~n) + 1;
}

// 🔟 Kiểm tra hai số có bằng nhau mà không dùng toán tử so sánh
bool is_equal(uint32_t a, uint32_t b) {
    return !(a ^ b);
}

// 1️⃣1️⃣ Tìm bit có trọng số lớn nhất (MSB)
int find_MSB_position(uint32_t n) {
    if (n == 0) return -1; // Không có bit nào bật
    int pos = 0;
    while (n >>= 1) ++pos;
    return pos;
}

// 1️⃣2️⃣ Tìm bit có trọng số nhỏ nhất (LSB)
...
// 1️⃣3️⃣ Tắt bit 1 ở ngoài cùng bên phải
...
// --- Hàm main demo ---
int main(void) {
    uint32_t n = 0b10100100;
    printf("Giá trị ban đầu: 0x%X\n", n);

    printf("\n1. Set bit 1 -> 0x%X", set_bit(n, 1));
    printf("\n2. Clear bit 2 -> 0x%X", clear_bit(n, 2));
    printf("\n3. Toggle bit 5 -> 0x%X", toggle_bit(n, 5));
    ...
    printf("7. Số bit 1 trong 0x%X là %d\n", n, count_set_bits(n));
    printf("8. 13 là %s\n", is_even(13) ? "chẵn" : "lẻ");
